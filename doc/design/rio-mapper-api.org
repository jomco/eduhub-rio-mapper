...
Het doorgeven van aanpassingen verloopt via de onderstaande endpoints:

- ~POST /job/upsert/education-specifications/{educationSpecificationId}~

- ~POST /job/upsert/programs/{programId}~

- ~POST /job/upsert/courses/{courseId}~


Verwijderen wordt doorgegeven via:

- ~POST /job/delete/education-specifications/{educationSpecificationId}~

- ~POST /job/delete/programs/{programId}~

- ~POST /job/delete/courses/{courseId}~
...
De "done" status bevat (sleutel-)attributen van het gemanipuleerde RIO
object.  Voorbeeld:
