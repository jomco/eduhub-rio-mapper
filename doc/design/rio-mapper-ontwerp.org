...
| 2022-10-03 | Webhooks                                          |
| 2023-07-04 | Fix sequence diagrams on RIO updates              |
| 2023-12-04 | Uitgebreide sequence diagrams voor alle jobs      |
...
** Mapper API Server
   ...
   *Interface*
   ...
   Het gebruikte endpoint is afhankelijk van het type hoofdobject.  In
   de URL is ~action~ één van:

   - ~upsert~ :: pas een al dan niet bestaand object aan

   - ~dry-run~ :: laat de resultaten van een upsert zien zonder deze uit te voeren

   - ~link~ :: pas de eigen sleutel aan van een object
   ...
   De "done" status bevat (sleutel-)attributen van het gemanipuleerde
   RIO object.  Voorbeeld:
...
De status van een job wordt opgehaald via het ~/status~ endpoint (zie
ook [[mapper-api-server]]) maar het is ook mogelijk een webhook door te
geven welke wordt aangeroepen als de job een bepaalde status heeft
bereikt.  De webhook wordt bij het aanmelden van de job doorgegeven
door middel van een HTTP header; ~X-Callback~.  Deze wordt aangeroepen
zodra de job een finale status bereikt (~done~, ~error~ of ~time-out~).

Er kan maar één webhook per job doorgeven worden.  Zodra de job de
status bereikt heeft, wordt een ~POST~ request uitgevoerd op de
doorgegeven URL met als body het status object zoals beschreven in
[[mapper-api-server]] met als extra attribuut "resource" welke verwijst
naar het bewerkte OOAPI object.

#+begin_example
POST https://example.com/rio-mapper/callback/5ec57c0de
Content-type: application/json
X-Rio-Mapper-Token: 23e45671-89be-2d31-456a-266554400004

{
  "status": "done",
  "attributes": {"opleidingseenheidcode": "1234"}
  "resource": "programs/123e4567-e89b-12d3-a456-426655440000"
}
#+end_example

Er worden alleen ~https~ URL's geaccepteerd en deze moet wijzen naar
een locatie waarvan de certificaten erkent worden door
[[https://openjdk.org/][OpenJDK]][fn:certs:Bijvoorbeeld van Let's Encrypt, Sectigo, Globalsign
of een andere bekende Certificaat Authoriteit.].  Er kunnen geen extra
headers voor dit request doorgegeven worden voor bijvoorbeeld
authenticatie, in plaats daarvan wordt er verwacht dat afnemers een
niet te raden *eigen* token aan de URL toevoegen om hiermee te
valideren dat het request echt is.


* Sequence diagrammen

** Update/insert happy flow
   ...
   #+begin_src plantuml :file (fig-file "diagrams/seq-happy") :exports results
     ...
     activate institute
       ...
       activate mapper
         ...
         mapper -> mapper : "Translate OO entity to RIO entity"


         mapper -> duo : "Update / insert RIO entity"
         activate duo
           duo --> mapper : "Ok"
         deactivate duo
         ...
         mapper -> gateway : "Fetch relations"
         activate gateway
      
           note over institute, mapper
             If the OO entity was an education specification
           end note

           gateway -> institute : "Fetch relations"
           activate institute
             institute --> gateway : "Relations for eduspec"
           deactivate institute
      
           gateway --> mapper : "Relations for eduspec"
         deactivate gateway

         mapper -> duo : "Insert/delete relation entities"
         activate duo
           note over mapper, duo
             For relations where both entities exist in RIO
           end note
           duo --> mapper : "Ok"
         deactivate duo

         mapper -> log : "Result for update_token"
         activate log
           note right
             RIO id inbegrepen
           end note
...
** Delete happy flow
   ...
   In [[fig:seq-delete-happy]] wordt geschetst welke acties er uitgevoerd
   worden bij een foutloze update.

   #+begin_src plantuml :file (fig-file "diagrams/seq-delete-happy") :exports results
     @startuml
     participant institute as "Instelling"
     participant gateway as "EduGateway"
     ...
     institute -> api : "OO entity deleted"
     activate institute
       activate api

         api -> api : "Generate token"

         api -> queue : "OO deleted with token"
         activate queue
           api --> institute : "Ok, response token"
         deactivate institute
       deactivate api

       queue -> mapper : "OO entity deleted with token"
       activate mapper

         institute -> api : "Get result for token"
         activate institute
           activate api

             api -> log : "Get result for token"
             activate log
               log --> api : null
             deactivate log
         ...
         mapper -> duo : "Delete relation entities"
         activate duo
           note over mapper, duo
             For relations that existed in RIO
           end note
           duo --> mapper : "Ok"
         deactivate duo

         mapper -> log : "Result for token"
         activate log
           log --> mapper : "Ok"
         deactivate log
         mapper --> queue : "Ack"
   ...
   #+end_src
   #+caption: Happy flow
   #+label: fig:seq-delete-happy
   #+results:
   [[file:diagrams/seq-delete-happy.png]]

   \clearpage
   
** Dry Run
 #+begin_src plantuml :file (fig-file "diagrams/seq-dry-run") :exports results
   @startuml
   participant institute as "Instelling"
   participant gateway as "EduGateway"
   ...
   institute -> api : "Dry run"
   activate institute
     activate api

       api -> api : "Generate token"

       api -> queue : "OO entity dry run with token"
       activate queue
         api --> institute : "Ok, response token"
       deactivate institute
     deactivate api

     queue -> mapper : "OO entity dry run with token"
     activate mapper
       ...
       mapper -> mapper : "Translate OO entity to Summary"

       mapper -> duo : "Fetch RIO entity"
       activate duo
         duo --> mapper : "RIO Entity data"
       deactivate duo

       mapper -> mapper : "Translate RIO entity to Summary"

       mapper -> log : "Diff of summaries"
       activate log
         log --> mapper : "Ok"
       deactivate log
       mapper --> queue : "Ack"
...
** Link / Unlink
 #+begin_src plantuml :file (fig-file "diagrams/seq-link") :exports results
   @startuml
   participant institute as "Instelling"
   ...
   activate institute
     activate api
       ...
       api -> queue : "Link (OOAPI Id, RIO Id) with token"
       activate queue
         api --> institute : "Ok, response token"
       deactivate institute
     deactivate api

     queue -> mapper : "Link (OOAPI Id, RIO Id) with token"
     note over mapper, api
       Unlink is link with null for OOAPI Id
     end note
     activate mapper
       mapper -> duo : "Fetch RIO entity"
       activate duo
       ...
       mapper -> mapper : "Get current Sleutel from Entity"

       mapper -> duo : "Set Sleutel to OOAPI Id for Entity"
       activate duo
         duo --> mapper : Ok
       deactivate duo

       mapper -> log : "Diff of Sleutels"
       activate log
         log --> mapper : "Ok"
       deactivate log
       mapper --> queue : "Ack"
...
*** Retryable
    ...
    #+begin_src plantuml :file (fig-file "diagrams/seq-retryable") :exports results
      ...
      mapper -> mapper : "Translate OO to RIO entity"
      mapper -> duo : "Update RIO entity"
      duo --> mapper : "Ok"
    ...
    #+results:
    [[file:diagrams/seq-retryable.png]]
