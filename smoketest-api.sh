...
PROGRAM_ID=49ca7998-74b1-f44a-1ec1-000000000003
...
# Create eduspec with ooapi-id $EDUCATION_SPECIFICATION_ID and rio-code generated by rio.
...
# Create program with ooapi-id $PROGRAM_ID and rio-code $PROGRAM_ID
...
# Remove ooapi id where program.rio-code == PROGRAM_ID
...
# Set ooapi id to $NEW_PROGRAM_ID where program.rio-code == PROGRAM_ID
NEW_PROGRAM_ID="4c358c84-dfc3-4a30-874f-$(xxd -l 6 -p /dev/urandom)"
URL="${ROOT_URL}/job/link/${PROGRAM_ID}/programs/${NEW_PROGRAM_ID}"
echo Link program
LINK_PROGRAM_TOKEN=$(curl -sf -X POST -H "Authorization: Bearer ${ACCESS_TOKEN}" "$URL" | jq -r .token)
echo "  token=$LINK_PROGRAM_TOKEN"
echo

# Delete program where ooapi_id == $NEW_PROGRAM_ID
URL="${ROOT_URL}/job/delete/programs/${NEW_PROGRAM_ID}"
